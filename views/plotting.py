...
the path trace via Plotly frames, so the animation runs in the browser -- no
server-side loop -- which keeps it smooth on Streamlit Community Cloud's free
tier. The regression figure is a static scatter + fitted line, with optional
residual segments to make the squared error visible.
"""
...
def regression_figure(x, y, slope, intercept, residuals=False):
    ...
    if residuals:
        ...
        fig.add_scatter(x=rx, y=ry, mode="lines",
